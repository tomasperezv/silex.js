  ...
  if (typeof this._map[urlParts.pathname] !== 'undefined') {
    ...
    route.map(function(current) {
      ...
      if (result instanceof Promise) {
        ...
        result.then(function(data, httpCode) {
          self._responseHandler.send(response, HTTP.OK, data);
        },
        function(errorMessage, httpCode) {
          self._responseHandler.send(response, HTTP.INTERNAL_ERROR, errorMessage);
        });
...
Router.prototype._processRoutePath = function(method) {
  ...
  var regexMatch = method.match(/(\/\w*\/)(\{([a-zA-Z,]*)\})/);
  var httpGetParams = {};
